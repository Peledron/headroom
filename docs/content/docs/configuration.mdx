...
# Keep an immutable warm prefix and compress only its live delta
headroom proxy --mode hybrid
...
| `HEADROOM_MODE` | Proxy optimization mode: `token`, `cache`, or `hybrid` | `token` |
| `HEADROOM_OPTICAL_TEXT` | Experimental Anthropic optical text conversion in `hybrid` mode. `1` enables cold-build and approved-rebase conversion only. Exact identifiers remain in a text sidecar. | `0` |
| `HEADROOM_OPTICAL_CACHE_DIR` | Content-addressed PNG cache for experimental optical text conversion | `~/.cache/headroom/text-optical` |
